...
local function setupDeathDetection()
    local function onCharacterAdded(character)
        ...
        if not success or not humanoid then
            warn("无法找到Humanoid: " .. tostring(err))
            return
        end
        ...
        humanoidDiedConnection = humanoid.Died:Connect(function()
            ...
            if anActivity then
                ...
                -- 更新GUI状态
                if _G.mainButton and _G.mainButton.Parent then
...
local function ProcessPart(v)
    ...
    if v:IsA("Part") and not v.Anchored and not v.Parent:FindFirstChild("Humanoid") and not v.Parent:FindFirstChild("Head") then
        if _G.processedParts[v] then
            ...
            if existingBV and existingBV.Parent then
                ...
                if _G.fixedMode 键，然后
                    ...
                    if existingBG 和 existingBG.Parent then
                        existingBG:Destroy()
        ...
        if v:FindFirstChild("Torque") 键，然后
            v.Torque:Destroy()
        end
